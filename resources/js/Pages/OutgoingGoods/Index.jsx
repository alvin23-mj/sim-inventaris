...
export default function Index({ transactions, currentMonth }) {
    ...
    const filteredTransactions = transactionsData.filter(tx => {
        ...
        const refMatch = tx.reference?.toLowerCase().includes(searchLower);
    ...
    const exportToCSV = async () => {
        ...
        // 2. Header
        const headerRow = worksheet.addRow(["Tanggal", "Tujuan / Unit", "Nama Peminta", "Daftar Barang", "Total Qty", "Petugas", "Catatan"], 5);
        ...
        saveAs(new Blob([buffer]), `Barang_Keluar_${currentMonth}.xlsx`);
    };

    return (
        <AuthenticatedLayout
            header={<h2 className="text-2xl font-bold leading-tight text-gray-800">Barang Keluar</h2>}
        >
                ...
                <div className="mb-4 flex flex-col lg:flex-row justify-between items-stretch lg:items-center bg-white p-4 sm:p-6 rounded-xl border border-gray-200 shadow-sm gap-4 no-print">
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                        <input 
                            type="month" 
                            value={currentMonth}
                            onChange={(e) => window.location.href = route('outgoing-goods.index', { month: e.target.value })}
                        ...
                        <div className="relative w-full sm:w-64">
                            ...
                            <input 
                                type="text" 
                                placeholder="Cari Tujuan atau Barang..."
                                value={search}
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                        ...
                        <CustomButton 
                            ...
                            className="justify-center"
                        >
                            <i className="fas fa-minus mr-2"></i> Catat Barang Keluar
                        </CustomButton>
                    </div>
                </div>
                        ...
                        <table className="min-w-full border-collapse border border-gray-300 whitespace-nowrap">
                            <thead className="bg-gray-100">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 text-left text-sm font-bold text-gray-700 border border-gray-300 uppercase">Tujuan / Unit</th>
                                    <th className="px-6 py-4 text-left text-sm font-bold text-gray-700 border border-gray-300 uppercase">Nama Peminta</th>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {filteredTransactions.map((tx) => (
                                    <tr key={tx.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-sm text-gray-700 font-semibold border border-gray-300">
                                            {tx.reference}
                                        </td>
                                                ...
                                                <tbody>
                                                    {tx.details.map((detail) => (
                                                        <tr key={detail.id} className="border-b border-gray-50 last:border-0">
                                                            ...
                                                            <td className="py-1 px-2 text-center font-bold text-red-600 w-12">{detail.quantity}</td>
                                        ...
                                        <td className="px-6 py-4 text-center text-sm font-bold text-red-600 border border-gray-300">
                                            {tx.details.reduce((sum, d) => sum + d.quantity, 0)}
                                        </td>
                                        ...
                                        <td className="px-6 py-4 text-center border border-gray-300">
                                            <div className="flex items-center justify-center">
                                                <button
                                                    onClick={() => window.open(route('outgoing-goods.print', tx.id), '_blank')}
